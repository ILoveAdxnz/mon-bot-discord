...
async def clear(ctx, amount: str, member: discord.Member = None):
    ...
    Exemples :
      +clear 8         → supprime les 8 derniers messages (tous utilisateurs)
      +clear 20 @user  → supprime les 20 derniers messages de @user
    ...
    if not is_all:
        try:
            ...
            return await ctx.send("❌ Utilise un nombre ou `all`. Exemple : `+clear 8` ou `+clear all`.", delete_after=5)
    ...
    if is_all:
        if member:
            def check_all_member(m): return m.author == member
            deleted = await ctx.channel.purge(limit=None, check=check_all_member, after=after_limit)
        else:
            deleted = await ctx.channel.purge(limit=None, after=after_limit)
    else:
        if member:
            # Filtrer uniquement les messages du membre ciblé
            def check_member(m): return m.author == member
            collected = []

            async for msg in ctx.channel.history(limit=2000, after=after_limit):
                if msg.author == member:
                    collected.append(msg)

                if len(collected) >= amount_int:
                    break
            ...
            # +clear 8 → supprime exactement les 8 derniers messages
            deleted = await ctx.channel.purge(limit=amount_int, after=after_limit)

    cible_str = f" de {member.mention}" if member else ""
    e = success_embed("🧹 Nettoyage", f"**{len(deleted)}** message(s) supprimé(s){cible_str}.\n**Modérateur :** {ctx.author.mention}")
...
async def snipe(ctx, member: discord.Member):
    """Afficher les messages supprimés d'un membre."""
    ...
    channel = ctx.guild.get_channel(latest["channel_id"])
    channel_name = channel.mention if channel else "Salon inconnu"

    e = warning_embed(
        f"🕵️ Snipe de {member}",
        latest["content"] or "[Message vide]"
    )

    e.add_field(name="Salon", value=channel_name, inline=True)

    try:
        ts = datetime.fromisoformat(latest["created_at"])
        e.add_field(name="Supprimé", value=f"<t:{int(ts.timestamp())}:R>", inline=True)
    except Exception:
        pass
