    ...
    if is_all:
        ...
        if member:
            ...
            if not collected:
                ...
                deleted = await ctx.channel.purge(limit=None, check=lambda m: m in collected)
        else:
            # +clear 8 → supprime exactement les 8 derniers messages
            messages = []

            async for msg in ctx.channel.history(limit=amount_int + 20, after=after_limit):
                if msg.id != ctx.message.id:
                    messages.append(msg)

                if len(messages) >= amount_int:
                    break
